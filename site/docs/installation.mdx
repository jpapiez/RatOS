...
:::
:::info Upgrading from RC3
If you are upgrading from 2.1.0-RC3, make sure you've read the [upgrade guide](/docs/upgrading_rc3/) first.
:::
...
Then use [RUFUS](https://rufus.ie/en/) (Windows only) or [Raspberry Pi Imager](https://www.raspberrypi.com/software/) to write that image to the SD card that goes into your raspberry pi (16GB or larger).
If you're using the Raspberry Pi Imager, make absolutely sure you've disabled custom/additional settings, or it will break the RatOS hotspot and network configuration. Problems are often reported when using [Balena Etcher](https://www.balena.io/etcher/) so it is no longer recommended.
...
:::info Update Manager Initialization
Occasionally, the Update Manager show may show one or more repositories as `INVALID`, often with a single `?` beneath the repository name. This is a Mainsail issue related to synchronising the system clock (time of day) using NTP. You can try to wait 2-3 minutes, then refresh. If the problem persists, restart the host (Raspberry Pi) and try again.
![Borked Update Manager](_media/update_manager_borked.png)
:::
